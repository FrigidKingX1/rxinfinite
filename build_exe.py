...
Output:
    Windows  -> dist/RXInfinite.exe
    Linux    -> dist/RXInfinite-linux-x86_64
...
if IS_WIN:
    APP_NAME = "RXInfinite"           # PyInstaller appends .exe
    OUTPUT_NAME = "RXInfinite.exe"
elif IS_LINUX:
    # Honest arch tag — CI publishes x86_64 only, but a local build on
    # aarch64 would otherwise produce a misnamed binary.
    APP_NAME = f"RXInfinite-linux-{platform.machine()}"
    OUTPUT_NAME = APP_NAME
else:
    APP_NAME = "RXInfinite"
    OUTPUT_NAME = "RXInfinite"
...
def build() -> None:
    ...
    # exactly where rxinfinite.py looks for them via _data_dir().
    # ------------------------------------------------------------------
    ...
    # the bundle root as logo.png; rxinfinite.py's _icon_path() looks there.
