...
These guard the regression where `RXInfinite.exe` (PyInstaller --windowed
...
RXINFINITE_PY = Path(__file__).resolve().parent.parent / "rxinfinite.py"


def _run_helper(op: str, hosts_file: Path, cfg_dir: Path) -> subprocess.CompletedProcess[str]:
    env = {
        **os.environ,
        "RXINFINITE_HOSTS_FILE": str(hosts_file),
        "XDG_CONFIG_HOME": str(cfg_dir),
        "APPDATA": str(cfg_dir),
    }
    return subprocess.run(
        [sys.executable, str(RXINFINITE_PY), "--admin-helper", op],
        env=env,
        capture_output=True,
        text=True,
        timeout=30,
...
def test_admin_helper_start_writes_block(temp_hosts: Path, tmp_path: Path) -> None:
    ...
    assert "# BEGIN RXINFINITE" in content
    assert "# END RXINFINITE" in content
...
def test_admin_helper_stop_removes_block(temp_hosts: Path, tmp_path: Path) -> None:
    ...
    assert "# BEGIN RXINFINITE" in temp_hosts.read_text(encoding="utf-8")

    result = _run_helper("stop", temp_hosts, cfg)
    assert result.returncode == 0, result.stderr
    content = temp_hosts.read_text(encoding="utf-8")
    assert "BEGIN RXINFINITE" not in content
    assert "END RXINFINITE" not in content
...
def test_admin_helper_start_is_idempotent(temp_hosts: Path, tmp_path: Path) -> None:
    ...
    assert _run_helper("start", temp_hosts, cfg).returncode == 0
    content = temp_hosts.read_text(encoding="utf-8")
    assert content.count("# BEGIN RXINFINITE") == 1
    assert content.count("# END RXINFINITE") == 1
...
def test_admin_helper_unknown_op_fails(temp_hosts: Path, tmp_path: Path) -> None:
    ...
    # Hosts unchanged.
    assert "BEGIN RXINFINITE" not in temp_hosts.read_text(encoding="utf-8")
...
def test_self_invocation_args_frozen(monkeypatch: pytest.MonkeyPatch) -> None:
    """Frozen builds re-launch RXInfinite.exe directly — never with a .py arg,
    because the PyInstaller bootloader doesn't know how to run one."""
    import importlib
    import rxinfinite as df
    monkeypatch.setattr(df.sys, "frozen", True, raising=False)
    monkeypatch.setattr(df.sys, "executable", "C:/whatever/RXInfinite.exe")
    args = df._self_invocation_args()
    assert args == ["C:/whatever/RXInfinite.exe"]
...
def test_self_invocation_args_source(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert args[1].endswith("rxinfinite.py")
    assert len(args) == 2
