...
Run as:  python integration_admin_helper_exe.py <path-to-RXInfinite[.exe]>

This is the test that would have caught the Win11 (and silent Win10) bug:
the prior elevation flow re-launched RXInfinite.exe with a `.py` helper
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        env = {
            **os.environ,
            "RXINFINITE_HOSTS_FILE": str(hosts),
        ...
        content = hosts.read_text(encoding="utf-8")
        if "BEGIN RXINFINITE" in content:
